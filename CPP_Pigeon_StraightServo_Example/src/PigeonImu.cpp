...
int PigeonImu::Set(ParamEnum paramEnum, double paramValue)
{
	uint64_t frame;
...
int PigeonImu::Set(ParamEnum paramEnum, TareType tareType, double angleDeg)
{
	...
	return Set(paramEnum, (double)paramValue);
}
int PigeonImu::SetYaw(double angleDeg)
{
	int errCode = Set(ParamEnum::YawOffset, TareType::SetValue, angleDeg);
...
{
	int errCode = Set(ParamEnum::YawOffset, TareType::AddOffset, angleDeg);
	return HandleError(errCode);
}
int PigeonImu::SetYawToCompass()
{
	int errCode = Set(ParamEnum::YawOffset, TareType::MatchCompass, 0);
	return HandleError(errCode);
}
int PigeonImu::SetFusedHeading(double angleDeg)
{
	int errCode = Set(ParamEnum::FusedHeadingOffset, TareType::SetValue, angleDeg);
	return HandleError(errCode);
}
/**
...
{
	int errCode = Set(ParamEnum::FusedHeadingOffset, TareType::AddOffset, angleDeg);
	return HandleError(errCode);
}
int PigeonImu::SetFusedHeadingToCompass()
{
	int errCode = Set(ParamEnum::FusedHeadingOffset, TareType::MatchCompass, 0);
	return HandleError(errCode);
}
/**
...
{
	int errCode = Set(ParamEnum::CompassOffset, TareType::SetOffset, 0);
	return HandleError(errCode);
}
/**
...
{
	int errCode = Set(ParamEnum::CompassOffset, TareType::SetValue, 0);
	return HandleError(errCode);
}
...
int PigeonImu::EnterCalibrationMode(CalibrationMode calMode)
{
	...
	frame <<= 8;
	frame |= (uint8_t) ParamEnum::EnterCalibration;
	int status = SendCAN(PARAM_SET | _deviceId, frame, 5, 0);
	return status;
}
/**
...
int PigeonImu::GetGeneralStatus(PigeonImu::GeneralStatus & statusToFill)
{
	...
	PigeonImu::CalibrationMode currentMode = (PigeonImu::CalibrationMode)(b5 >> 4);
...
int PigeonImu::ReceiveCAN(int arbId, bool allowStale /*= true*/)
{
	_len = 0;
	_cache = 0;
